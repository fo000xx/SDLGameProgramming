...
bool LTexture::loadFromRenderedText(std::string textureText, SDL_Color textColour, LWindow& currentWindow)
{
    resetTextureParams();

    auto* rawTextSurface = TTF_RenderText_Solid(mFont.get(), textureText.c_str(), textColour);
    if (rawTextSurface == NULL) {
        std::cout << "Unable to render text surface! SDL_ttf Error: " << TTF_GetError() << '\n';
        return false;
    }
    std::unique_ptr<SDL_Surface, customDeleter<SDL_Surface>> RAIITextSurface{ rawTextSurface };

    auto* newRawTexture = SDL_CreateTextureFromSurface(&currentWindow.getRenderer(), RAIITextSurface.get());
    if (newRawTexture == NULL) {
        std::cout << "Unable to create texture. SDL Error: " << SDL_GetError() << '\n';
        return false;
    }
    
    mTextureWidth = RAIITextSurface->w;
    mTextureHeight = RAIITextSurface->h;
