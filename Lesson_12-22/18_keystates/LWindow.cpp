...
bool LWindow::loadMedia(std::array<LTexture*, gConstants::numOfTextures>& textureArray, std::array<std::string, gConstants::numOfTextures>& pathArray)
//bool LWindow::loadMedia(LTexture& texture, std::string& path)
{
    for (int i { 0 }; i < gConstants::numOfTextures; ++i) {
        if (!textureArray[i]->loadFromFile(pathArray[i], *this)) {
            std::cout << "failure to load texture " << i << '\n';
            return false;
        }
    }
