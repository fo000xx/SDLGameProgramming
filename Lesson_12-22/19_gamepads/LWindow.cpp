...
LWindow::LWindow()
{
        ...
        if (SDL_NumJoysticks() < 1) {
            throw std::runtime_error("Warning: no joysticks connected!");
        }
        } catch (const std::exception& exception) {
            std::cout << "Standard Exception: " << exception.what() << '\n';
        }     
        
        mGameController.reset(SDL_JoystickOpen(0));
        try {
        if (mGameController == NULL) {
            throw std::runtime_error(std::string("Warning: Unable to open game contorller! SDL Error: ") + SDL_GetError());
        }
        } catch (const std::exception& exception) {
            std::cout << "Standard Exception: " << exception.what() << '\n';
        }
